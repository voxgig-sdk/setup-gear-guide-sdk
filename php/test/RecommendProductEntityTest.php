...
class RecommendProductEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SETUP_GEAR_GUIDE_TEST_RECOMMEND_PRODUCT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $recommend_product_ref01_data = Helpers::to_map(is_object($recommend_product_ref01_data_result) && method_exists($recommend_product_ref01_data_result, 'data_get') ? $recommend_product_ref01_data_result->data_get() : $recommend_product_ref01_data_result);
...
function recommend_product_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("SETUP_GEAR_GUIDE_TEST_RECOMMEND_PRODUCT_ENTID");
