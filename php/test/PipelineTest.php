...
// SetupGearGuide SDK pipeline test
//
...
{
    private static function utility(): SetupGearGuideUtility
    {
        return new SetupGearGuideUtility();
    }
    ...
    private static function ctx(array $over = []): SetupGearGuideContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new SetupGearGuideContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new SetupGearGuideOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof SetupGearGuideError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new SetupGearGuideOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = SetupGearGuideMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new SetupGearGuideOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = SetupGearGuideMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new SetupGearGuideOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = SetupGearGuideMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SetupGearGuideSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = SetupGearGuideMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new SetupGearGuideResponse([]),
            'result' => new SetupGearGuideResult([]),
        ]);
        [, $err] = SetupGearGuideMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new SetupGearGuideSpec([]),
            'response' => null,
            'result' => new SetupGearGuideResult([]),
        ]);
        [, $err] = SetupGearGuideMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new SetupGearGuideSpec(['step' => 's']),
            'response' => new SetupGearGuideResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new SetupGearGuideResult([]),
        ]);
        [, $err] = SetupGearGuideMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new SetupGearGuideSpec(['step' => 's']),
            'response' => new SetupGearGuideResponse(self::resp(200, ['v' => 1])),
            'result' => new SetupGearGuideResult([]),
        ]);
        [, $err] = SetupGearGuideMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new SetupGearGuideSpec(['step' => 's']),
            'response' => new SetupGearGuideResponse(self::resp(200, ['v' => 2])),
            'result' => new SetupGearGuideResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        SetupGearGuideMakeResponse::call($ctx);
    ...
    {
        $preset = new SetupGearGuideResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new SetupGearGuideSpec([]),
            'response' => new SetupGearGuideResponse([]),
            'result' => new SetupGearGuideResult([]),
        ...
        [$got, $err] = SetupGearGuideMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new SetupGearGuideResult([])]);
        [, $err] = SetupGearGuideMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new SetupGearGuideSpec([]), 'result' => null]);
        [, $err] = SetupGearGuideMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SetupGearGuideSpec(['step' => 's']),
            'result' => new SetupGearGuideResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new SetupGearGuideOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SetupGearGuideMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SetupGearGuideSpec(['step' => 's']),
            'result' => new SetupGearGuideResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new SetupGearGuideOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SetupGearGuideMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SetupGearGuideResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new SetupGearGuideSpec([]),
            'result' => new SetupGearGuideResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = SetupGearGuideMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SetupGearGuideMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new SetupGearGuideError('boom', 'boom');
        $utility->fetcher = function (SetupGearGuideContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SetupGearGuideSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = SetupGearGuideMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (SetupGearGuideContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = SetupGearGuideMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(SetupGearGuideResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        SetupGearGuideMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (SetupGearGuideContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SetupGearGuideSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = SetupGearGuideMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new SetupGearGuideResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new SetupGearGuideSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = SetupGearGuideMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = SetupGearGuideMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = SetupGearGuideMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new SetupGearGuideResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, SetupGearGuideDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new SetupGearGuideResult(['ok' => false])]);
        $this->expectException(SetupGearGuideError::class);
        SetupGearGuideDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new SetupGearGuideResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', SetupGearGuideMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        SetupGearGuideMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new SetupGearGuideBaseFeature();
        $b = new SetupGearGuideBaseFeature();
        SetupGearGuideFeatureAdd::call($ctx, $a);
        SetupGearGuideFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): SetupGearGuideBaseFeature
    {
        $f = new SetupGearGuideBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        SetupGearGuideFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): SetupGearGuideContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new SetupGearGuideSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SetupGearGuidePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        SetupGearGuidePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new SetupGearGuideResponse(['headers' => null]),
            'result' => new SetupGearGuideResult([]),
        ]);
        SetupGearGuideResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        SetupGearGuideResultBody::call($ctx);
