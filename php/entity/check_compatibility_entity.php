...
// SetupGearGuide SDK CheckCompatibility entity
...
class CheckCompatibilityEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "check_compatibility";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new CheckCompatibilityEntity($this->_client, $opts);
    }

    /**
     * @param CheckCompatibility|array $args CheckCompatibility data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = SetupGearGuideHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CheckCompatibility|array The current CheckCompatibility data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CheckCompatibility fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = SetupGearGuideHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CheckCompatibility fields).
     */
    ...
    /**
     * Load a single CheckCompatibility.
     *
     * @param CheckCompatibilityLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed CheckCompatibilityLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CheckCompatibility|array The loaded CheckCompatibility as an assoc-array at the
     *   SDK boundary; throws SetupGearGuideError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = SetupGearGuideHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * Create a new CheckCompatibility.
     *
     * @param CheckCompatibilityCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed CheckCompatibilityCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CheckCompatibility|array The created CheckCompatibility as an assoc-array at the
     *   SDK boundary; throws SetupGearGuideError on failure (item-5 convention).
     */
