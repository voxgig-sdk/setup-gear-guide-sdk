...
// SetupGearGuide SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class SetupGearGuideTestFeature extends SetupGearGuideBaseFeature
{
    ...
    public function init(SetupGearGuideContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (SetupGearGuideContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = SetupGearGuideParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (SetupGearGuideContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(SetupGearGuideContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = SetupGearGuideParam::call($ctx, $k);
