...
const __1 = require("..");
const SDK_NAME = 'SetupGearGuideSDK';
...
const TEST_SEED = { "entity": { "build_quote": { "test01": { "id": "test01" } }, "check_compatibility": { "test01": { "id": "test01" } }, "compare_product": { "test01": { "id": "test01" } }, "get_affiliate_offer": { "test01": { "id": "test01" } }, "get_build": { "test01": { "id": "test01" } }, "get_product": { "test01": { "id": "test01" } }, "recommend_product": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+SetupGearGuideSDK\b/.test(code) ||
        /\bSetupGearGuideSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+SetupGearGuideSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/SetupGearGuideSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
