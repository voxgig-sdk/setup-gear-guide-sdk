...
describe('BuildQuoteDirect', async () => {
  ...
  test('direct-load-build_quote', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-build_quote', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SETUP_GEAR_GUIDE_TEST_BUILD_QUOTE_ENTID']
