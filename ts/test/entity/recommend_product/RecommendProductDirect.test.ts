...
describe('RecommendProductDirect', async () => {
  ...
  test('direct-load-recommend_product', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-recommend_product', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SETUP_GEAR_GUIDE_TEST_RECOMMEND_PRODUCT_ENTID']
