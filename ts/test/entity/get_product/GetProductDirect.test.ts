...
describe('GetProductDirect', async () => {
  ...
  test('direct-load-get_product', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_product', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SETUP_GEAR_GUIDE_TEST_GET_PRODUCT_ENTID']
