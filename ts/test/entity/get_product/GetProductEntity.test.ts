...
describe('GetProductEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'get_product.' + op, live)) return
    }
    ...
    const get_product_ref01_data_dt0 = await get_product_ref01_ent.load(get_product_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['SETUP_GEAR_GUIDE_TEST_GET_PRODUCT_ENTID']
