...
describe('CheckCompatibilityDirect', async () => {
  ...
  test('direct-load-check_compatibility', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-check_compatibility', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SETUP_GEAR_GUIDE_TEST_CHECK_COMPATIBILITY_ENTID']
