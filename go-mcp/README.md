# setup-gear-guide-mcp

[MCP](https://modelcontextprotocol.io) server exposing the SetupGearGuide SDK as
two agent tools — `setup-gear-guide_list` and `setup-gear-guide_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/setup-gear-guide-mcp)
make build

# 2. Provide credentials via the environment
export SETUP_GEAR_GUIDE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user setup-gear-guide \
  -- /absolute/path/to/setup-gear-guide-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./setup-gear-guide-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// setup-gear-guide_list: first page of records
{ "entity": "build_quote" }
{ "entity": "build_quote", "query": { } }

// setup-gear-guide_load: one record by id
{ "entity": "build_quote", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/setup-gear-guide-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user setup-gear-guide \
     -- "$PWD"/dist/*/setup-gear-guide-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `setup-gear-guide_list` and `setup-gear-guide_load` tools now appear
   in new sessions. Ask the agent to *"list build_quote using setup-gear-guide"*
   and it calls `setup-gear-guide_list` with `{"entity":"build_quote"}`.
...
```sh
export SETUP_GEAR_GUIDE_APIKEY=sk_live_xxx            # API key
export SETUP_GEAR_GUIDE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `setup-gear-guide_list` tool
...
### Call the `setup-gear-guide_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `setup-gear-guide_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `setup-gear-guide_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SETUP_GEAR_GUIDE_APIKEY` | API key sent with every request. |
| `SETUP_GEAR_GUIDE_BASE` | Optional override of the API base URL. |

### Entities

The 7 entities valid as the `entity` argument:

build_quote | check_compatibility | compare_product | get_affiliate_offer | get_build | get_product | recommend_product

### Smoke test via HTTP (raw JSON-RPC)

```sh
./setup-gear-guide-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"setup-gear-guide_load","arguments":{"entity":"build_quote","query":{"id":1}}}}'
```
