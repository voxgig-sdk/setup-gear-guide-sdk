...
type Args struct {
	Entity string         `json:"entity" jsonschema:"build_quote | check_compatibility | compare_product | get_affiliate_offer | get_build | get_product | recommend_product"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.SetupGearGuideSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "setup-gear-guide_list",
		Description: "List records from SetupGearGuide. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "setup-gear-guide_load",
		Description: "Load a single record from SetupGearGuide. " +
...
func runOp(client *sdk.SetupGearGuideSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.SetupGearGuideSDK, name string) (sdk.SetupGearGuideEntity, error) {
	switch strings.ToLower(name) {
	case "build_quote":
		return client.BuildQuote(nil), nil
	case "check_compatibility":
		return client.CheckCompatibility(nil), nil
	case "compare_product":
		return client.CompareProduct(nil), nil
	case "get_affiliate_offer":
		return client.GetAffiliateOffer(nil), nil
	case "get_build":
		return client.GetBuild(nil), nil
	case "get_product":
		return client.GetProduct(nil), nil
	case "recommend_product":
		return client.RecommendProduct(nil), nil
