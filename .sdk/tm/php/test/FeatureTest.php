...
// SetupGearGuide SDK feature test
//
...
class FtCtrl extends SetupGearGuideControl
{
...
class FtClient
{
    ...
    private SetupGearGuideUtility $utility;

    public function __construct(SetupGearGuideUtility $utility, array $options)
    {
    ...
    public function get_utility(): SetupGearGuideUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SetupGearGuideContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public SetupGearGuideContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (SetupGearGuideContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SetupGearGuideConfig::make_feature($name);
    ...
    private static function buildUrl(SetupGearGuideSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SetupGearGuideError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SetupGearGuideResponse($fetched);
        }
    ...
    private function fail(SetupGearGuideContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SetupGearGuideContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
