...
//    only - a typed SetupGearGuideContext there would make every ctx
...
final class SetupGearGuideOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new SetupGearGuideContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new SetupGearGuideSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new SetupGearGuideResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new SetupGearGuideError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new SetupGearGuideResponse($clean['response']);
    ...
    public static function err_from_map($m): ?SetupGearGuideError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new SetupGearGuideError(is_string($code) ? $code : '', $msg);
    }
