...
// SetupGearGuide SDK retry feature
...
class SetupGearGuideRetryFeature extends SetupGearGuideBaseFeature
{
    ...
    public function init(SetupGearGuideContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SetupGearGuideContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SetupGearGuideContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SetupGearGuideContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
