...
module SetupGearGuideUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SetupGearGuideResponse.new({})
    ...
    if fetch_err
      ...
      response = SetupGearGuideResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SetupGearGuideResponse.new(fetched)
    else
