...
local BuildQuoteEntity = {}
BuildQuoteEntity.__index = BuildQuoteEntity


function BuildQuoteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BuildQuoteEntity)
  self._name = "build_quote"
  self._client = client
...
function BuildQuoteEntity:get_name()
  return self._name
end


function BuildQuoteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BuildQuoteEntity.new(self._client, opts)
end
...
function BuildQuoteEntity:mark_deleted()
  self._deleted = true
end


function BuildQuoteEntity:deleted()
  return true == self._deleted
end


function BuildQuoteEntity:data_set(args)
  if args ~= nil then
...
function BuildQuoteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BuildQuoteEntity:match_set(args)
  if args ~= nil then
...
function BuildQuoteEntity:match_get()
...
function BuildQuoteEntity:stream(action, args, callopts)
...
---@param reqmatch BuildQuoteLoadMatch
---@param ctrl? table
---@return BuildQuote
---@return string? err
function BuildQuoteEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata BuildQuoteCreateData
---@param ctrl? table
---@return BuildQuote
---@return string? err
function BuildQuoteEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function BuildQuoteEntity:_run_op(ctx, post_done)
  local utility = self._utility
