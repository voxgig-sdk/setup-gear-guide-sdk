...
local CompareProductEntity = {}
CompareProductEntity.__index = CompareProductEntity


function CompareProductEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompareProductEntity)
  self._name = "compare_product"
  self._client = client
...
function CompareProductEntity:get_name()
  return self._name
end


function CompareProductEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompareProductEntity.new(self._client, opts)
end
...
function CompareProductEntity:mark_deleted()
  self._deleted = true
end


function CompareProductEntity:deleted()
  return true == self._deleted
end


function CompareProductEntity:data_set(args)
  if args ~= nil then
...
function CompareProductEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompareProductEntity:match_set(args)
  if args ~= nil then
...
function CompareProductEntity:match_get()
...
function CompareProductEntity:stream(action, args, callopts)
...
---@param reqmatch CompareProductLoadMatch
---@param ctrl? table
---@return CompareProduct
---@return string? err
function CompareProductEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata CompareProductCreateData
---@param ctrl? table
---@return CompareProduct
---@return string? err
function CompareProductEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function CompareProductEntity:_run_op(ctx, post_done)
  local utility = self._utility
