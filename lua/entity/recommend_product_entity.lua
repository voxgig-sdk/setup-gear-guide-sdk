...
local RecommendProductEntity = {}
RecommendProductEntity.__index = RecommendProductEntity


function RecommendProductEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RecommendProductEntity)
  self._name = "recommend_product"
  self._client = client
...
function RecommendProductEntity:get_name()
  return self._name
end


function RecommendProductEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RecommendProductEntity.new(self._client, opts)
end
...
function RecommendProductEntity:mark_deleted()
  self._deleted = true
end


function RecommendProductEntity:deleted()
  return true == self._deleted
end


function RecommendProductEntity:data_set(args)
  if args ~= nil then
...
function RecommendProductEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RecommendProductEntity:match_set(args)
  if args ~= nil then
...
function RecommendProductEntity:match_get()
...
function RecommendProductEntity:stream(action, args, callopts)
...
---@param reqmatch RecommendProductLoadMatch
---@param ctrl? table
---@return RecommendProduct
---@return string? err
function RecommendProductEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata RecommendProductCreateData
---@param ctrl? table
---@return RecommendProduct
---@return string? err
function RecommendProductEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function RecommendProductEntity:_run_op(ctx, post_done)
  local utility = self._utility
