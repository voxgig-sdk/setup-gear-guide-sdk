...
class CheckCompatibilityEntity
  ...
  def make
    opts = @_entopts.dup
    CheckCompatibilityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = SetupGearGuideHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [CheckCompatibility, Hash] the current CheckCompatibility data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = SetupGearGuideHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of CheckCompatibility fields)
  def match_get
  ...
  #
  # @param reqmatch [CheckCompatibilityLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.CheckCompatibility.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [CheckCompatibility, Hash] the loaded CheckCompatibility; raises SetupGearGuideError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = SetupGearGuideHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqdata [CheckCompatibilityCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [CheckCompatibility, Hash] the created CheckCompatibility; raises SetupGearGuideError on failure
