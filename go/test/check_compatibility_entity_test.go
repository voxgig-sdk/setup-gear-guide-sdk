...
func TestCheckCompatibilityEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SETUPGEARGUIDE_TEST_CHECK_COMPATIBILITY_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		checkCompatibilityRef01Data = core.ToMapAny(checkCompatibilityRef01DataResult)
...
func check_compatibilityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SETUPGEARGUIDE_TEST_CHECK_COMPATIBILITY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SETUPGEARGUIDE_TEST_CHECK_COMPATIBILITY_ENTID"])
