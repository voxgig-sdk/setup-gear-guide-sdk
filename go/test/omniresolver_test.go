...
import (
	...
	omni "github.com/voxgig-sdk/setup-gear-guide-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.SetupGearGuideError); is {
				out := map[string]any{
					"name":    "SetupGearGuideError",
...
func omniCtx(arg any, client *sdk.SetupGearGuideSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.SetupGearGuideSDK); is {
			client = live
