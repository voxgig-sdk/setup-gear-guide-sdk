...
func TestBuildQuoteEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SETUPGEARGUIDE_TEST_BUILD_QUOTE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		buildQuoteRef01Data = core.ToMapAny(buildQuoteRef01DataResult)
...
func build_quoteBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SETUPGEARGUIDE_TEST_BUILD_QUOTE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SETUPGEARGUIDE_TEST_BUILD_QUOTE_ENTID"])
