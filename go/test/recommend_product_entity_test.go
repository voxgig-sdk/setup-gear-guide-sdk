...
func TestRecommendProductEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SETUPGEARGUIDE_TEST_RECOMMEND_PRODUCT_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		recommendProductRef01Data = core.ToMapAny(recommendProductRef01DataResult)
...
func recommend_productBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SETUPGEARGUIDE_TEST_RECOMMEND_PRODUCT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SETUPGEARGUIDE_TEST_RECOMMEND_PRODUCT_ENTID"])
