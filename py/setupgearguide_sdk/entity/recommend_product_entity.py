...
class RecommendProductEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RecommendProductEntity(self._client, opts)
    ...
    def data_get(self) -> RecommendProduct:
    ...
    def match_get(self) -> RecommendProduct:
    ...
    def load(self, reqmatch=None, ctrl=None) -> RecommendProduct:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RecommendProduct().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: RecommendProductCreateData, ctrl=None) -> RecommendProduct:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
