# SetupGearGuide SDK test runner
...
class SetupGearGuideTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SetupGearGuideTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SetupGearGuideTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SetupGearGuideTestRunner.getenv("SETUP_GEAR_GUIDE_TEST_LIVE")
        override = SetupGearGuideTestRunner.getenv("SETUP_GEAR_GUIDE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SetupGearGuideTestRunner.getenv(key)
        ...
        explain = SetupGearGuideTestRunner.getenv("SETUP_GEAR_GUIDE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SETUP_GEAR_GUIDE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if SetupGearGuideTestRunner._test_control is not None:
            return SetupGearGuideTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SetupGearGuideTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SetupGearGuideTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SetupGearGuideTestRunner.load_test_control()
...
def load_env_local():
    SetupGearGuideTestRunner.load_env_local()


def env_override(m):
    return SetupGearGuideTestRunner.env_override(m)


def entity_data(v):
    return SetupGearGuideTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SetupGearGuideTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SetupGearGuideTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SetupGearGuideTestRunner.load_test_control()


def live_client_options():
    return SetupGearGuideTestRunner.live_client_options()


def live_delay_ms():
    return SetupGearGuideTestRunner.live_delay_ms()
