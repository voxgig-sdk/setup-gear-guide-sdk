    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SETUP_GEAR_GUIDE_TEST_BUILD_QUOTE_ENTID JSON to run live")
        client = setup["client"]
        ...
        build_quote_ref01_data = helpers.to_map(runner.entity_data(build_quote_ref01_ent.create(build_quote_ref01_data, None)))
...
def _build_quote_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("SETUP_GEAR_GUIDE_TEST_BUILD_QUOTE_ENTID"))
