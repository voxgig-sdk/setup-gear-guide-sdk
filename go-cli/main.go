// Command SetupGearGuide-cli is a CLI + REPL for the SetupGearGuide SDK,
...
const prompt = "setup-gear-guide"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "build_quote check_compatibility compare_product get_affiliate_offer get_build get_product recommend_product"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SETUP_GEAR_GUIDE_APIKEY (api key), SETUP_GEAR_GUIDE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SETUP_GEAR_GUIDE_APIKEY carries the API key and
	// SETUP_GEAR_GUIDE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SETUP_GEAR_GUIDE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SETUP_GEAR_GUIDE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSetupGearGuideSDK(opts)
