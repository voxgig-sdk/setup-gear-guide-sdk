# setup-gear-guide-cli

boru-driven command-line client **and** interactive REPL for the SetupGearGuide
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/setup-gear-guide-cli)
make build

# 2. See usage (words, entities, env vars)
./setup-gear-guide-cli --help

# 3. Provide credentials once, via the environment
export SETUP_GEAR_GUIDE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./setup-gear-guide-cli load 1 build_quote            # {id:1} shorthand
./setup-gear-guide-cli load '{id:1}' build_quote       # explicit match map

# 5. Override the API base URL for a single call
SETUP_GEAR_GUIDE_BASE=https://api.example.com ./setup-gear-guide-cli load 1 build_quote

# 6. No arguments -> interactive REPL
./setup-gear-guide-cli
setup-gear-guide> load 1 build_quote
setup-gear-guide> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/setup-gear-guide-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/setup-gear-guide-cli load 1 build_quote
   ```
...
```sh
./setup-gear-guide-cli load 1 build_quote          # scalar shorthand for {id:1}
./setup-gear-guide-cli load '{id:1}' build_quote     # explicit match map
```
...
./setup-gear-guide-cli load 1 build_quote
```
...
Run with no arguments to open a REPL (prompt `setup-gear-guide>`). Each line is
evaluated as its own boru expression:

```text
$ ./setup-gear-guide-cli
setup-gear-guide> load 1 build_quote
setup-gear-guide> /help
setup-gear-guide> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `build_quote`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/setup-gear-guide-cli`. |
...
(`./setup-gear-guide-cli <expr>`) and interactively (the REPL), and expressions compose the
